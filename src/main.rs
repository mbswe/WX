...
fn create_locations_config(path: &std::path::Path) {
    ...
    locations
        .push_str("\"Effectsoft Halmstad\" = { latitude = 56.676086, longitude = 12.858977 }\n");
...
fn pretty_print_weather_data(weather_data: WeatherData) {
    ...
    for item in weather_data.properties.timeseries.iter() {
        ...
        println!(
            ...
            if item.data.instant.details.air_temperature.is_some() {
                item.data.instant.details.air_temperature.unwrap()
            } else {
                0.0
            },
            if item.data.instant.details.relative_humidity.is_some() {
                item.data.instant.details.relative_humidity.unwrap()
            } else {
                0.0
            },
            if item.data.instant.details.wind_speed.is_some() {
                item.data.instant.details.wind_speed.unwrap()
            } else {
                0.0
            },
            if item.data.instant.details.wind_from_direction.is_some() {
                item.data.instant.details.wind_from_direction.unwrap()
            } else {
                0.0
            },
            if item
